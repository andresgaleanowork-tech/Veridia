  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      if (patientsRes.status === 'fulfilled') {
        setStats((s) => ({ ...s, totalPatients: patientsRes.value.data.total || 0 }));
        setRecentPatients(patientsRes.value.data.data?.slice(0, 5) || []);
      }
      if (apptsRes.status === 'fulfilled') {
        setTodayAppts(apptsRes.value.data || []);
        setStats((s) => ({ ...s, todayAppointments: (apptsRes.value.data || []).length }));
      }
      if (invoicesRes.status === 'fulfilled') {
        setStats((s) => ({ ...s, pendingInvoices: invoicesRes.value.data.total || 0 }));
      }
