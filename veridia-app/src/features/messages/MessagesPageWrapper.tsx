...
import { MessageSquare, RefreshCw, Home, Mail } from 'lucide-react';
...
function MessagesPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar los mensajes. Intenta recargar o contacta a soporte.
        </p>
...
export function MessagesPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <MessagesPageFallback {...props} />}
    >
      <MessagesPage />
    </ErrorBoundary>
  );
