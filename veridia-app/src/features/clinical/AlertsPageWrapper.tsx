...
function AlertsPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <h1 className="text-2xl font-bold text-text mb-2 text-center">
          Error en Alertas Clínicas
        </h1>

        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar las alertas. Intenta recargar o contacta a soporte.
        </p>
...
export function AlertsPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <AlertsPageFallback {...props} />}
    >
      <AlertsPage />
    </ErrorBoundary>
  );
