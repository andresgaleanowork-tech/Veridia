...
import { EspenPage } from './EspenPage';
import { Shield, RefreshCw, Home, Mail } from 'lucide-react';
...
function EspenPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <h1 className="text-2xl font-bold text-text mb-2 text-center">
          Error en Guías ESPEN
        </h1>

        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar las guías. Intenta recargar o contacta a soporte.
        </p>
        ...
        <div className="flex gap-3">
          {navigate && (
            <button
              onClick={() => navigate('/clinical/espen')}
...
export function EspenPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <EspenPageFallback {...props} />}
    >
      <EspenPage />
    </ErrorBoundary>
  );
