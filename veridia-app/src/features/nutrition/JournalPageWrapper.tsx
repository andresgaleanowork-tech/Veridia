...
import { BookOpen, RefreshCw, Home, Mail } from 'lucide-react';
...
function JournalPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <h1 className="text-2xl font-bold text-text mb-2 text-center">
          Error en Diario de Alimentación
        </h1>

        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar el diario de alimentación. Intenta recargar o contacta a soporte.
        </p>
        ...
        <div className="flex gap-3">
          {navigate && (
            <button
              onClick={() => navigate('/nutrition/journal')}
...
export function JournalPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <JournalPageFallback {...props} />}
    >
      <JournalPage />
    </ErrorBoundary>
  );
