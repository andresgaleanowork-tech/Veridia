...
import { Calendar, RefreshCw, Home, Mail } from 'lucide-react';
...
function MealPlansPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <h1 className="text-2xl font-bold text-text mb-2 text-center">
          Error en Planes de Alimentación
        </h1>

        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar los planes. Intenta recargar o contacta a soporte.
        </p>
...
export function MealPlansPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <MealPlansPageFallback {...props} />}
    >
      <MealPlansPage />
    </ErrorBoundary>
  );
