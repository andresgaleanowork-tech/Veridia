...
import { Utensils, RefreshCw, Home, Mail } from 'lucide-react';
...
function FoodsPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar la base de alimentos. Intenta recargar o contacta a soporte.
        </p>
...
export function FoodsPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <FoodsPageFallback {...props} />}
    >
      <FoodsPage />
    </ErrorBoundary>
  );
