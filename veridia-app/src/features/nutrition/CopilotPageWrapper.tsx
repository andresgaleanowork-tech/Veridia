...
function CopilotPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar el asistente. Intenta recargar o contacta a soporte.
        </p>
        ...
        <div className="flex gap-3">
          {navigate && (
            <button
              onClick={() => navigate('/nutrition/copilot')}
...
export function CopilotPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <CopilotPageFallback {...props} />}
    >
      <CopilotPage />
    </ErrorBoundary>
  );
