...
function RecipesPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar las recetas. Intenta recargar o contacta a soporte.
        </p>
...
export function RecipesPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <RecipesPageFallback {...props} />}
    >
      <RecipesPage />
    </ErrorBoundary>
  );
