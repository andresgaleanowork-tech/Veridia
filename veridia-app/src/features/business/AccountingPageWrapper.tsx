...
import { DollarSign, RefreshCw, Home, Mail } from 'lucide-react';
...
function AccountingPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar la contabilidad. Intenta recargar o contacta a soporte.
        </p>
...
export function AccountingPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <AccountingPageFallback {...props} />}
    >
      <AccountingPage />
    </ErrorBoundary>
  );
