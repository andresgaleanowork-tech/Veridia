...
function AppointmentsPageFallback({ error, requestId, resetError, isDev, navigate, copyErrorDetails, reportError }: FallbackProps) {
    ...
    >
      ...
      >
        ...
        <p className="text-text-3 text-sm mb-6 text-center">
          No se pudo cargar el calendario. Intenta recargar o contacta a soporte.
        </p>
...
export function AppointmentsPageWrapper() {
  return (
    <ErrorBoundary
      fallbackRender={(props) => <AppointmentsPageFallback {...props} />}
    >
      <AppointmentsPage />
    </ErrorBoundary>
  );
