    ...
    <Dialog open={open} onClose={onClose} title="Nueva Factura">
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={mutation.isPending} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-[#0891B2] to-[#0E7490] rounded-lg text-sm font-bold text-white hover:shadow-lg transition-all disabled:opacity-50">{mutation.isPending ? 'Creando...' : 'Crear Factura'}</button>
        </div>
      </form>
    </Dialog>
  );
