...
export function IntegrationsPage() {
  ...
  const { data: webhooks } = useQuery({
    queryKey: ['webhooks'],
    queryFn: async () => { const res = await api.get('/webhooks'); return res.data || []; },
  });

  const createApiKeyMutation = useMutation({
    mutationFn: async (data: { name: string; scopes?: string[] }) => { const res = await api.post('/webhooks', data); return res.data; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['api-keys'] }); addToast('success', 'API Key creada'); setShowApiKeyDialog(false); },
  });

  const createWebhookMutation = useMutation({
    mutationFn: async (data: { url: string; events: string[]; secret?: string }) => { const res = await api.post('/webhooks', data); return res.data; },
