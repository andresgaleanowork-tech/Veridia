...
export interface CareProcess {
  id: string;
  paciente_id: string;
  current_step: CareProcessStep;
  steps_completed: CareProcessStep[];
  screening_data: {
    tool: string;
    score: number;
  ...
  reevaluation_data?: Record<string, unknown>;
...
export type CareProcessStep = 'screening' | 'assessment' | 'diagnosis' | 'intervention' | 'followup' | 'reevaluation';
...
export interface OnboardingSubmission {
  ...
  waiversSigned: Record<string, unknown>;
  completedAt?: string;
}

// Patient Context Hub types
export type {
  PatientComputedState, NCPStatus, GLIMDiagnosis, ESPENTargets, PNENPrescription,
  DrugNutrientAlert, PrecisionTargets, AdherenceRiskScore, EDScreeningResult,
  SportsNutritionProfile, NutrigenomicProfile, MicrobiomeProfile, EatingBehaviorProfile,
  BioactivesProfile, PlanetaryHealthScore, Demographics, Anthropometry, LabValues,
  GLIMSeverity, AlertSeverity, AlertStatus, AdherenceRiskLevel, EDRiskLevel,
