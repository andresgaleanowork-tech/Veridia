...
export function AppointmentFormDialog({ open, onClose }: Props) {
  ...
  const { data: providers } = useQuery({
    queryKey: ['providers'],
    queryFn: async () => {
      return await api.getUnwrapped<Provider[]>('/providers') ?? [];
    },
    enabled: open,
  });
  ...
  const eventColor = selectedProvider?.colorCalendar || watch('color') || '#4A7C59';
    ...
    <Dialog open={open} onClose={onClose} title="Nueva Cita">
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={mutation.isPending} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-primary to-primary-dark rounded-lg text-sm font-bold text-white hover:shadow-lg transition-all disabled:opacity-50">{mutation.isPending ? 'Creando...' : 'Crear Cita'}</button>
        </div>
      </form>
    </Dialog>
  );
