    ...
    <Dialog open={open} onClose={onClose} title="Nuevo Gasto">
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={mutation.isPending} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-primary to-primary-dark rounded-lg text-sm font-bold text-white hover:shadow-lg transition-all disabled:opacity-50">{mutation.isPending ? 'Creando...' : 'Crear Gasto'}</button>
        </div>
      </form>
    </Dialog>
  );
