...
export function PatientsPage() {
  ...
  const { data: patients, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Patient[]> => {
      ...
      const data = await api.getUnwrapped<Patient[]>(`/patients?${params.toString()}`);
      return data ?? [];
    },
  });

  const handleDelete = useCallback(async (row: Patient) => {
  ...
  const actions = useMemo<TableAction<Patient>[]>(() => [
    { label: 'Ver', icon: <Eye size={14} />, onClick: (row) => navigateTo(`/patients/${row.id}`) },
    { label: 'Editar', icon: <Edit size={14} />, onClick: (row) => setEditingPatient(row) },
    { label: 'Eliminar', icon: <Trash2 size={14} />, variant: 'danger', onClick: (row) => handleDelete(row) },
  ], [handleDelete, navigateTo]);

  const handleEditClose = () => setEditingPatient(null);
...
<div className="flex gap-3 flex-wrap">
      ...
      <PatientFormDialog open={!!editingPatient} onClose={handleEditClose} initialData={editingPatient} title="Editar Paciente" />
    </div>
  );
}