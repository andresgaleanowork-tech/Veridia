...
import {
  ...
  Stethoscope, FileSpreadsheet, Brain, Pill
} from 'lucide-react';
...
type TabId = 'overview' | 'clinical' | 'anthropometry' | 'analytics' | 'appointments' | 'messages' | 'fitness' | 'context' | 'supplements';
...
const TABS: TabDef[] = [
  ...
  { id: 'context', label: 'Context Hub', icon: <Brain size={14} /> },
  { id: 'supplements', label: 'Suplementos', icon: <Pill size={14} /> },
];
...
export function PatientDetailPage() {
  ...
  const { data: patient, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Patient> => {
      return await api.getUnwrapped<Patient>(`/patients/${id}`);
    },
    enabled: !!id,
  });
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {activeTab === 'context' && id && (
          <ContextHubTab patientId={id} />
        )}

        {activeTab === 'supplements' && id && (
          <SupplementsPage patientId={id} />
        )}
      </div>
