...
export function TenantsPage() {
  ...
  const { data: tenants, isLoading } = useQuery({
    queryKey: ['tenants'],
    queryFn: async () => { return await api.getUnwrapped<Tenant[]>('/tenants') ?? []; },
  });

  const createMutation = useMutation({
    mutationFn: async (data: TenantFormData) => { return await api.post('/tenants', data); },
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={showDialog} onClose={() => setShowDialog(false)} title="Nuevo Tenant">
        <TenantForm onSubmit={(data) => createMutation.mutate(data)} onCancel={() => setShowDialog(false)} />
      </Dialog>
    </div>
  );
}

function TenantForm({ onSubmit, onCancel }: TenantFormProps) {
