...
export class PatientContextHub {
  ...
  private scopedEventBuses = new Map<string, any>();
  private computationStats = new Map<string, any>();
   ...
   */
  registerModule(_module: any): void {
    void _module;
  ...
  private async loadPatientBaseData(patientId: string): Promise<any> {
  ...
  private async buildComputedState(baseContext: any): Promise<PatientComputedState> {
    const state: PatientComputedState = {
      ...
      anthropometry: baseContext.anthropometry || {},
  ...
  private updateStats(patientId: string, durationMs: number, wasCacheHit: boolean): void {
    const stats = this.computationStats.get(patientId) || {
      totalComputations: 0,
      totalCacheHits: 0,
  ...
  private createChecksum(data: any): string {
    ...
    const str = typeof data === 'object' ? JSON.stringify(data, Object.keys(data).sort()) : String(data);
